...
(function() {
  this.iNote = function(domContainer) {
    var CELLS, _VER, _bindKeys, _container, _init, _keyMap, _themeName, appendNewCell, clear, createNewCell, deleteCell, findNextEditableCell, findPrevEditableCell, fullScreen, getCellInfo, getIndexByCellNumber, getKeyMap, getNewCellNumber, getTheme, getXmlText, getXmlText0, getXmlText1, globalKeyHandler, inserCellAfter, insertCellBefore, isCellFirst, isCellLast, moveFocusToNextCell, moveFocusToPreviousCell, runCell, setKeyMap, setTheme, setXmlText, setXmlText0, setXmlText1;
    ...
    globalKeyHandler = function(event) {
      ...
      } else if (key === 40 && cell.cursorOnLastLine()) {
        moveFocusToNextCell(index);
      }
    };
    ...
    findNextEditableCell = function(index) {
      var c, j, len, ref;
      if (index >= CELLS.length - 1) {
        return null;
      }
      ref = CELLS.slice(index + 1);
    ...
    findPrevEditableCell = function(index) {
      var c, i, j, ref;
      if (index <= 0) {
        return null;
      }
      for (i = j = ref = index - 1; ref <= 0 ? j <= 0 : j >= 0; i = ref <= 0 ? ++j : --j) {
        c = CELLS[i];
        if (c.isEditable()) {
          return c;
        }
